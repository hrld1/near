...
### Iteración 31: "El estreno" — Near sale a internet (gratis y escalable)
El hueco nº 1 de la auditoría: parejas reales. Tras comparar las capas gratuitas de 2026, se eligió una arquitectura que **no toca el diseño**: Northflank (proceso siempre encendido — el bus SSE en memoria lo exige) + Neon (Postgres) + Cloudflare R2 (fotos, despierta el adaptador S3 de la it6) + Cloudflare Realtime TURN (llamadas) + VAPID. Todo en capa gratuita; el camino de escalado (subir plan de cómputo → despertar el bus Redis ya escrito) queda documentado.

El criterio de elección fue **que el contenedor no hiberne**, y descarta a la mayoría: las capas gratuitas de Render y de la antigua de Koyeb bajan a cero tras 15–60 min sin tráfico, lo que cada noche tiraría la presencia, las rachas en vivo y las partidas en curso. Koyeb retiró además su cómputo gratuito en 2026 (adquirida por Mistral AI), y la migración a Northflank se hizo sin tocar una línea de la aplicación — solo la imagen de destino y tres variables del CI.
- **Imagen de producción**: `Dockerfile` multi-stage (build standalone de Next), entrypoint que aplica `prisma migrate deploy` antes de arrancar, y `/api/health` con estado de la BD y versión desplegada. Verificado en local de punta a punta (imagen, migraciones, health, login).
- **TURN de Cloudflare, dormido sin claves**: `/api/ice` (con sesión) genera credenciales caducas por API y las cachea; la llamada las recoge en paralelo al permiso de cámara, con el fallback STUN de siempre. Cierra el riesgo de la llamada P2P en CGNAT sin coste.
- **CI/CD**: GitHub Actions — push a `main` → lint+tipos+tests → imagen en GHCR → redeploy en Northflank (se omite sin secretos, no falla). Backup semanal cifrado (`pg_dump` → GPG AES-256 → R2, poda a 60 días) y uptime cada 10 min contra `/api/health` que abre/cierra un issue solo.
- **`DEPLOY.md`**: guía paso a paso (ruta Northflank gratis + ruta VPS con compose/Caddy), la semilla separa contenido de la pareja demo (`SEED_DEMO=false` en producción), y los E2E de Playwright honran `BASE_URL` para correr contra el despliegue real.
- **Estado y privacidad** en Ajustes: versión desplegada, aviso de piloto y — sin adornos — que el chat aún no está cifrado de extremo a extremo. Preferimos decirlo a que se dé por hecho.
- Verificación: 126 tests (3 nuevos de ICE), build, E2E locales en verde y captura real de la página de estado.
