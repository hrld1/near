...
El hueco nº 1 de la auditoría: parejas reales. Tras comparar las capas gratuitas de 2026, se eligió una arquitectura que **no toca el diseño**: Koyeb (proceso siempre encendido — el bus SSE en memoria lo exige) + Neon (Postgres) + Cloudflare R2 (fotos, despierta el adaptador S3 de la it6) + Cloudflare Realtime TURN (llamadas) + VAPID. Todo en capa gratuita; el camino de escalado (subir instancia → despertar el bus Redis ya escrito) queda documentado.
...
- **CI/CD**: GitHub Actions — push a `main` → lint+tipos+tests → imagen en GHCR → redeploy en Koyeb (se omite sin secretos, no falla). Backup semanal cifrado (`pg_dump` → GPG AES-256 → R2, poda a 60 días) y uptime cada 10 min contra `/api/health` que abre/cierra un issue solo.
- **`DEPLOY.md`**: guía paso a paso (ruta Koyeb gratis + ruta VPS con compose/Caddy), la semilla separa contenido de la pareja demo (`SEED_DEMO=false` en producción), y los E2E de Playwright honran `BASE_URL` para correr contra el despliegue real.
